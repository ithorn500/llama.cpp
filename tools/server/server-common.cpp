...
#endif
{
    ...
    llama_pos new_n_past = pos; // unused for now
#if defined(GG_GATEWAY_EPIC514_SERVER_MTMD) && GG_GATEWAY_EPIC514_SERVER_MTMD
...
#endif
    const char * external_device = std::getenv("GEMMA_ENGINE_MMPROJ_EXTERNAL_DEVICE");
    if (external_device && std::string(external_device) == "npu") {
        LOG_ERR("NPU mmproj hook did not process %s chunk; refusing CPU/GPU fallback", name);
        n_tokens_out = 0;
        return -514;
    }
